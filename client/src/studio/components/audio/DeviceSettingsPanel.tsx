        ...
        <div className="grid gap-6 py-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="grid gap-2">
              <Label htmlFor="mic-select" className="text-foreground flex items-center gap-2" title="Escolha o dispositivo de entrada para gravação.">
                Microfone de Entrada
                {micState && <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Microfone ativo" />}
              </Label>
              <Select
              ...
              >
                ...
                <SelectContent className="bg-popover border-border text-popover-foreground">
                  ...
                  {inputDevices.map((device) => (
                    <SelectItem key={device.deviceId || `input-${Math.random()}`} value={device.deviceId || `input-${device.label || 'unknown'}`}>
                      <div className="flex items-center gap-2">
                        <span>{device.label || `Microfone ${device.deviceId?.slice(0, 5) || 'default'}`}</span>
                        {micState && device.deviceId === micState.stream.getAudioTracks()[0]?.getSettings()?.deviceId && (
              ...
              {micState && (
                <div className="text-xs text-muted-foreground">
                  Dispositivo ativo: {micState.stream.getAudioTracks()[0]?.label || "Microfone padrão"}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={async (val) => {
                  ...
                  if (!granted) return;
                  onSettingsChange({ ...settings, outputDeviceId: val });
                }}
              >
                ...
                <SelectContent className="bg-popover border-border text-popover-foreground">
                  ...
                  {outputDevices.map((device) => {
                    ...
                    return (
                      <SelectItem key={device.deviceId || `output-${Math.random()}`} value={device.deviceId || `output-${device.label || 'unknown'}`}>
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              <Label className="text-foreground flex items-center gap-2" title="Ajusta sensibilidade de entrada do microfone.">
                ...
                Ganho de Entrada
                {settings.inputGain > 1.5 && <span className="text-xs text-amber-500 font-medium">Alto</span>}
                {settings.inputGain < 0.5 && <span className="text-xs text-blue-500 font-medium">Baixo</span>}
              </Label>
            ...
            {settings.inputGain === 0 && (
              <div className="text-xs rounded-md border border-amber-500/30 bg-amber-500/10 text-amber-300 px-2 py-1.5 flex items-center gap-2">
                <AlertTriangle className="w-3.5 h-3.5" />
                Microfone está mudo - nenhum áudio será capturado
              </div>
            )}
            {settings.inputGain > 1.8 && (
              <div className="text-xs rounded-md border border-red-500/30 bg-red-500/10 text-red-300 px-2 py-1.5 flex items-center gap-2">
                <AlertTriangle className="w-3.5 h-3.5" />
                Ganho muito alto pode causar distorção no áudio
              </div>
            )}
          </div>
          ...
          <div className="grid gap-4">
            <div className="flex items-center justify-between">
              <Label className="text-foreground flex items-center gap-2" title="Controla volume local de retorno para monitoramento durante a sessão.">
                ...
                Volume do Monitor
                {settings.monitorVolume === 0 && <span className="text-xs text-red-500 font-medium">Mudo</span>}
                {settings.monitorVolume > 0.8 && <span className="text-xs text-amber-500 font-medium">Alto</span>}
              </Label>
              <span className="text-xs font-mono text-primary">
            ...
            {settings.monitorVolume === 0 && (
              <div className="text-xs rounded-md border border-blue-500/30 bg-blue-500/10 text-blue-300 px-2 py-1.5 flex items-center gap-2">
                <Volume2 className="w-3.5 h-3.5" />
                Volume de monitor mudo - você não ouvirá o áudio de playback
              </div>
            )}
